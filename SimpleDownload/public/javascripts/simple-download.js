...
(function ($) {
    ...
    function onloadHander($frame, $form, callback, context) {
        return function () {
            // 在文档开始下载后
            // 使用给定的上下文调用回调
    ...
    $.download = {
        'post': function (url, param, callback, context) {
            ...
            var $frame = createIframe(id).on('load', onloadHander($frame, $form, callback, context));
            $('body').append($form.append($hiddens), $frame);
            $form.submit();
        },
        'get': function (url, param, callback, context) {
            var id = newId();
            var $frame = createIframe(id).on('load', onloadHander($frame, null, callback, context));
            $('body').append($frame);
